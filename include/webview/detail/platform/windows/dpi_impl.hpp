...
#ifndef WEBVIEW_DETAIL_PLATFORM_WINDOWS_DPI_IMPL_HPP
#define WEBVIEW_DETAIL_PLATFORM_WINDOWS_DPI_IMPL_HPP
...
{
  WEBVIEW_IMPL bool is_per_monitor_v2_awareness_available()
  {
    // Windows 10, version 1703
    return compare_os_version(10, 0, 15063) >= 0;
  }

  WEBVIEW_IMPL bool enable_dpi_awareness()
  {
  ...
  WEBVIEW_IMPL bool enable_non_client_dpi_scaling_if_needed(const HWND window)
  {
    const auto user32 = native_library(L"user32.dll");
  ...
  WEBVIEW_IMPL int get_window_dpi(const HWND window)
  {
    const auto user32 = native_library(L"user32.dll");
  ...
  WEBVIEW_IMPL SIZE make_window_frame_size(const HWND window, const int width, const int height, const int dpi)
  {
