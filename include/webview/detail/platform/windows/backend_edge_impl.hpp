...
#ifndef WEBVIEW_DETAIL_PLATFORM_WINDOWS_BACKEND_EDGE_IMPL_HPP
#define WEBVIEW_DETAIL_PLATFORM_WINDOWS_BACKEND_EDGE_IMPL_HPP
...
namespace webview::detail
{
  ...
  WEBVIEW_IMPL webview2_com_handler::webview2_com_handler(const HWND hwnd, msg_cb_t msgCb, webview2_com_handler_cb_t cb)
      : m_window(hwnd),
  ...
  WEBVIEW_IMPL ULONG STDMETHODCALLTYPE webview2_com_handler::AddRef()
  {
    return ++m_ref_count;
  }

  WEBVIEW_IMPL ULONG STDMETHODCALLTYPE webview2_com_handler::Release()
  {
    if (m_ref_count > 1)
      return --m_ref_count;
  ...
  WEBVIEW_IMPL HRESULT STDMETHODCALLTYPE webview2_com_handler::QueryInterface(REFIID riid, LPVOID* ppv)
  {
  ...
  WEBVIEW_IMPL HRESULT STDMETHODCALLTYPE webview2_com_handler::Invoke(HRESULT res, ICoreWebView2Environment* env)
  {
    if (SUCCEEDED(res))
    {
  ...
  WEBVIEW_IMPL HRESULT STDMETHODCALLTYPE webview2_com_handler::Invoke(const HRESULT res, ICoreWebView2Controller* controller)
  {
    if (FAILED(res))
    {
  ...
  WEBVIEW_IMPL HRESULT STDMETHODCALLTYPE webview2_com_handler::Invoke(ICoreWebView2* /*sender*/, ICoreWebView2WebMessageReceivedEventArgs* args)
  {
    LPWSTR message{};
  ...
  WEBVIEW_IMPL HRESULT STDMETHODCALLTYPE webview2_com_handler::Invoke(ICoreWebView2* /*sender*/, ICoreWebView2PermissionRequestedEventArgs* args)
  {
  ...
  WEBVIEW_IMPL void webview2_com_handler::set_attempt_handler(std::function<HRESULT()> attempt_handler) noexcept
  {
    m_attempt_handler = std::move(attempt_handler);
  }

  WEBVIEW_IMPL void webview2_com_handler::try_create_environment() noexcept
  {
  ...
  WEBVIEW_IMPL webview2_user_script_added_handler::webview2_user_script_added_handler(callback_fn cb)
      : m_cb(std::move(cb))
  {
  }

  WEBVIEW_IMPL ULONG STDMETHODCALLTYPE webview2_user_script_added_handler::AddRef()
  {
    return ++m_ref_count;
  }

  WEBVIEW_IMPL ULONG STDMETHODCALLTYPE webview2_user_script_added_handler::Release()
  {
    if (m_ref_count > 1)
      return --m_ref_count;
  ...
  WEBVIEW_IMPL HRESULT STDMETHODCALLTYPE webview2_user_script_added_handler::QueryInterface(REFIID riid, LPVOID* ppv)
  {
    using namespace mswebview2::cast_info;
  ...
  WEBVIEW_IMPL HRESULT STDMETHODCALLTYPE webview2_user_script_added_handler::Invoke(const HRESULT res, const LPCWSTR id)
  {
    m_cb(res, id);
    return S_OK;
  }
  ...
  WEBVIEW_IMPL win32_edge_engine::win32_edge_engine() {}
  ...
  WEBVIEW_IMPL void win32_edge_engine::dispatch_impl(dispatch_fn_t f)
  {
  ...
  WEBVIEW_IMPL noresult win32_edge_engine::navigate_impl(const std::string& url)
  {
  ...
  WEBVIEW_IMPL void win32_edge_engine::set_title_impl(const std::string& title)
  {
    SetWindowTextW(m_window, widen_string(title).c_str());
  }

  WEBVIEW_IMPL noresult win32_edge_engine::add_page_init_script(const std::string& js)
  {
    const auto wide_js = widen_string(js);
    bool done{};
  ...
  WEBVIEW_IMPL noresult win32_edge_engine::on_window_opened_impl()
  {
  ...
  WEBVIEW_IMPL void win32_edge_engine::run_event_loop_while(const std::function<bool()>& fn)
  {
    MSG msg;
  ...
  WEBVIEW_IMPL noresult win32_edge_engine::window_settings()
  {
    auto cb = std::bind(&win32_edge_engine::on_message, this, std::placeholders::_1);
    return embed(m_widget, cb);
  }

  WEBVIEW_IMPL void win32_edge_engine::window_show()
  {
  ...
  WEBVIEW_IMPL noresult win32_edge_engine::embed(const HWND wnd, msg_cb_t cb)
  {
  ...
  WEBVIEW_IMPL void win32_edge_engine::resize_widget() const
  {
    if (m_widget)
    {
      RECT r{};
  ...
  WEBVIEW_IMPL void win32_edge_engine::resize_webview() const
  {
  ...
  WEBVIEW_IMPL void win32_edge_engine::focus_webview() const
  {
    if (m_controller)
      m_controller->MoveFocus(COREWEBVIEW2_MOVE_FOCUS_REASON_PROGRAMMATIC);
  }

  WEBVIEW_IMPL bool win32_edge_engine::is_webview2_available() const noexcept
  {
  ...
  WEBVIEW_IMPL void win32_edge_engine::on_dpi_changed(const int dpi)
  {
  ...
  WEBVIEW_IMPL SIZE win32_edge_engine::get_size() const
  {
    RECT bounds;
  ...
  WEBVIEW_IMPL SIZE win32_edge_engine::get_scaled_size(const int from_dpi, const int to_dpi) const
  {
  ...
  WEBVIEW_IMPL void win32_edge_engine::on_system_setting_change(const wchar_t* area) const
  {
