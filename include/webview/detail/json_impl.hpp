...
#ifndef WEBVIEW_DETAIL_JSON_IMPL_HPP
#define WEBVIEW_DETAIL_JSON_IMPL_HPP
...
{
  WEBVIEW_IMPL int json_parse_c(const char* str, size_t size, const char* key, size_t key_size, const char** value, size_t* value_size)
  {
    enum
    {
      JSON_STATE_VALUE,
  ...
  WEBVIEW_IMPL std::string json_escape(const std::string& s, const bool add_quotes)
  {
  ...
  WEBVIEW_IMPL int json_unescape(const char* s, size_t n, char* out)
  {
    int r = 0;
    if (*s++ != '"')
      return -1;
  ...
  WEBVIEW_IMPL std::string json_parse(const std::string& s, const std::string& key, const int index)
  {
    const char* value;
    size_t value_size;
