...
type MinimalOutputBundle = Record<
  string,
  MinimalOutputAsset | MinimalOutputChunk
>;
...
function transformPublicFile(publicFile: PublicDirFile) {
  ...
  const bytes = [...fs.readFileSync(publicFile.fullPath)]
    .map((v) => String(v))
    .join(",");
...
function writeHeader(
  ...
  outputDir?: string,
  options?: CppHeaderPluginOptions,
  publicDir?: string,
  devServerPort?: number,
) {
  const outputPath =
    options?.outputPath ?? path.join(outputDir ?? "dist", "ViteAssets.h");
...
export interface CppHeaderPluginOptions {
  outputPath?: string;
  includeFileEnumeration?: boolean;
}

export default function pluginCppHeader(
  options?: CppHeaderPluginOptions,
): Plugin {
  let writeServerActive = false;
  let writeBundleActive = false;
  let publicDir: string | undefined = "public";
  ...
  return {
    ...
    config(userOptions: UserConfig, env) {
      ...
      } else if (userOptions.publicDir === false) {
        publicDir = undefined;
      }
    },
